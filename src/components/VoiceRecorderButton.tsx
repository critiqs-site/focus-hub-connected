...
const VoiceRecorderButton = ({ onTranscript }: VoiceRecorderButtonProps) => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2">
      {state === "recording" && (
        <div className="px-3 py-1 rounded-full bg-destructive/90 text-destructive-foreground text-xs font-medium flex items-center gap-1.5 animate-pulse shadow-lg">
          <span className="w-1.5 h-1.5 rounded-full bg-current" /> {fmt(seconds)}
        </div>
      )}
      <button
        onClick={handleClick}
